...
export const ReceiverStatus: React.FC = () => {
  ...
  const fetchReceivers = async (overrides?: any) => { 
      ...
      try {
        const query = {
            marketName: overrides?.marketName ?? searchMarket,
            macAddress: overrides?.macAddress ?? searchMac
        };
  ...
  const handleSearch = () => {
      setIsFiltered(true);
      fetchReceivers({ marketName: searchMarket, macAddress: searchMac });
  };
  ...
  const handleReset = () => {
      ...
      setSearchMac('');
      setIsFiltered(false);
      fetchReceivers({ marketName: '', macAddress: '' });
  };
          ...
          <>
             ...
             {loading ? (
                 <div className="py-20 text-center text-slate-500 flex flex-col items-center gap-2">
                    <RefreshCw className="animate-spin" /> 데이터 로딩중...
                 </div>
             ) : (
                 <DataTable 
                    columns={[
